...
acceptable.  Build and record all four audit buckets below, using the provided
...
- **Lifecycle / workflow edges**: enumerate each changed state transition,
  callback pair, setup/teardown edge, success/failure return, and async/remove
  interaction that can enter or leave the changed logic; prove cleanup/state
  ownership for every edge before writing `SAFE`.
...
**Mandatory generic bug-class audit**: when the diff shape matches, explicitly
inspect and record proof for setup/helper returns that flow into publish/
register/add paths, stack/local addresses stored into longer-lived state,
selector/count admission checks that may omit a peer dimension, duplicate
shutdown/unregister/release paths, and failed-start / failed-resume stale-state
contamination. Also apply the source-confirmed audits for managed-resource
manual cleanup, static descriptors retaining freed dynamic objects, one-shot
control replay, shared packet buffers, level-triggered IRQ re-enable, and
alternate TPG/internal-source paths when the diff shape matches.
...
- **Step 3d (DT/DT-binding) / Step 3d.3 (driver `of_*`)**: for DT
  contract changes (Step 3d), use DTS context files to verify sibling property
  indentation and phandle targets; for driver `of_*` API changes (Step 3d.3), use context to
...
- `<project_path>/tmp/sparse_<slug>.txt` — sparse run once at REVIEW_TIP
  ...
  this patch touches. If a daemon/runtime override disabled sparse, this file
  instead contains the single line `(sparse disabled by config)` and should be
  treated as a deliberate `SKIP`, not as a missing artifact
- `<project_path>/tmp/patch_<N>_dtbinding.txt` — dt_binding_check / dtbs_check
  output; read only if the file exists (absent means no DT schema/DTS files
  were changed by this patch). For `.dts/.dtsi` patches, this artifact is
  scoped to the resolved DTB targets affected by the patch, not a full-tree
  arm64 `dtbs_check` dump.
...
applies when a DT schema, DTS/DTSI, or `include/dt-bindings/` header is changed,
and its YAML/DTS sub-checklists are loaded only for the matching changed file
surface. Step 3d.3 (driver `of_*` API) applies when driver code uses the DT API,
and only the triggered checklist is included in this brief. When their trigger
conditions ARE present, they are equally mandatory and cannot be skipped. The
output section headers are unconditional: every commit block must include
`DT / DT-Binding Notes` and `Hardware Engineering Notes`; when the trigger is
absent, write the explicit `Not applicable: ...` body shown below.
...
- **Severity cap**: findings for unreviewable files are capped at
  ...
  rather than filed as [BUG].  This prevents false positives.
...
This avoids overclaiming on files that were not available for review and signals
the orchestrator to re-run with more context.
...
<div class="commit-block">
  ...
  <div class="commit-body">
    ...
    <pre>codebase audit: entrypoints ...
codebase audit: callees ...
codebase audit: siblings ...
control-flow: ...
data-flow: ...
state/lifecycle: workflow/ownership edge matrix ...
before-vs-after delta: ...</pre>

    <!-- ALWAYS include this section.  When neither Step 3d (DT schema/DTS/header
         contract changes) nor Step 3d.3 (driver of_match/of_* API changes) is
         triggered, the body is exactly:
           <li>Not applicable: no DT binding, .dts*, dt-bindings header, or of_match changes
               in this patch.</li>
    ...
    <!-- One .finding-card per [BUG] or [CONCERN] finding.  Every card MUST
         ...
         index and <K> is the 1-based finding index within this block.  It
         should also carry data-attribution="introduced" or
         data-attribution="newly_exposed".  Never use
         data-attribution="pre_existing_only" on a finding-card; those belong
         in .preexisting-note only.  Do not use finding-cards for self-negating,
         process-only, or informational notes; route those to Positive Notes or
         other non-counted prose instead. -->
    <div class="finding-card bug" id="patch-N-finding-1" data-attribution="introduced">
  ...
  -   removed_line;</pre>
    ...
    <!-- OPTIONAL: use only when you need to mention a real issue that exists
         in unchanged surrounding code and is NOT introduced by this patch.
         These notes are context/follow-up only: they do not affect the
         verdict banner, stat chips, or finding-card totals. -->
    <h3>Pre-existing Issues (non-blocking)</h3>
    <div class="preexisting-note">
      <span class="preexisting-flag">Pre-existing</span>
      Short summary. Present in unchanged code near the patch; not introduced
      by this patch. Mention the file/location and suggest a follow-up patch
      if useful.
    </div>
    ...
    <!-- One .finding-card per [MINOR] or [NIT] finding.  Continue the same
         ...
         this block. -->
    <div class="finding-card minor" id="patch-N-finding-K" data-attribution="introduced">
    ...
    <div class="positive-note">Positive observation or non-blocking review note about the patch.</div>
...
- Every `.finding-card` must have all six sub-elements in order:
  ...
  `Commit message`.
- Every normal review `.finding-card` should carry `data-attribution` with one
  of these values: `introduced` for a defect created by the current patch, or
  `newly_exposed` when the patch makes a pre-existing harmful path newly
  reachable, required, user-visible, or materially worse.  Do not emit
  `pre_existing_only` as a finding-card value; use `.preexisting-note` instead.
  During migration the validator still falls back to prose checks when the
  attribute is absent, but when the attribute is present it is the source of
  truth for `pre_existing_scope`.
...
- Use the optional `<h3>Pre-existing Issues (non-blocking)</h3>` section only
  for issues in unchanged surrounding code that are explicitly **not**
  introduced by the current patch.  Render them as `.preexisting-note`, not as
  `.finding-card`.
- Only actionable code/style issues belong in `.finding-card`.  If the text
  says the current code is correct/safe/acceptable/not a defect, or the point
  is merely "for completeness" / "worth verifying", keep it in `Positive Notes`
  or patch-local prose instead of filing a counted finding.
- Pre-existing notes are patch-local context only.  They MUST NOT appear in
  the verdict banner, MUST NOT change the stat-chip counts, and MUST NOT be
  used to justify `NEEDS FIXES` / `NEEDS DISCUSSION` for an otherwise clean
  patch.  The validator rejects `Pre-existing` content inside `.finding-card`
  title/body/file-ref/suggestion fields as `pre_existing_scope` when no
  structured `data-attribution` is available; structured attribution is
  preferred because prose can be ambiguous.
- The `<h3>Issues</h3>`, `<h3>Minor / Style</h3>`, and
  `<h3>Positive Notes</h3>` sections are always present even when empty.  The
  pre-existing section is optional and should be omitted when there are no such
  notes.
...
- `codebase_audit` is mandatory in every block.  Use
  `DONE entrypoints=<n> callees=<n> siblings=<n> files=[...]` for patches with
  function-level code changes.  The `files=[...]` list MUST name the actual
  files inspected during the surrounding-code audit, including provided context
  files, targeted reads, and every function-level `required_reads[].path` from
  the evidence manifest.  `siblings=<n>` may be `0` only when the Code Logic
  Maps line explicitly says no sibling or alternate path exists.  Use `N/A no
  function-level code changes` for DT-binding-only, dt-bindings-header-only,
  documentation, Kconfig, Makefile, or other non-function patches that have no
  `function_level_source_files`, changed functions, or helper candidates in the
  evidence manifest.
- `evidence_manifest` is mandatory when an Evidence file is provided.  It must
  ...
  path safety, DT schema coverage, or surrounding-code completeness.  Required
  reads for non-function artifacts (for example DT schemas and
  `include/dt-bindings/**` headers) may be proved by `on_demand_reads` when
  `codebase_audit` is `N/A`; do not invent entrypoint/callee/sibling audit
  lines for artifacts that contain no function-level code.
...
- The structural checks `scripts/validate_review.py` runs in Step 6.7 are
  unbypassable: a missing Gate trace, missing STEP_COMPLETION_RECORD field,
  missing DT / Hardware Engineering section header, missing or malformed
  ...
  finding text slots, pre-existing issues emitted as findings,
  hardware-looking changes marked N/A, incomplete rate-refactor coverage
  matrices, current-safe future-risk concerns, safe/no-action BUG-or-CONCERN
  findings, low-severity crash/state-regression findings, helper-equivalence
  claims made without source-backed proof, missing patch-corpus validation
  in daemon mode, touched unchecked runtime-PM gets, alternate paths
  bypassing a new resource/rate/power abstraction, unproven
  helper-replacement postconditions, or parent/child compatible-contract
  mismatches will cause the validator to reject the report and force
  self-repair/re-spawn.

**Step 5d — Validation trace requirement (mandatory for every finding):**

Every canonical per-commit `.finding-card` MUST include in its `.body` div the
parenthetical validation trace defined in `refs/gate-rules.md` (the
`Trace requirement` block immediately after the THREE-GATE RULE section, plus
the always-[BUG] and `[NIT]` variants). Use the trace formats and
`[sub-rule: ...]` tag rules exactly as specified there. Place the trace in
`.body` only — never in `.suggestion` and never after a sibling `<pre>` block.
A finding without the applicable validation trace is invalid.
...
value is anything other than `normal`, apply the adjustments documented in
`special-cases.md` (loaded with this brief) — covering merge commits, revert
patches, RFC patches, whitespace-only patches, and documentation-only patches.
Never skip mandatory steps from this file regardless of patch type; the
`special-cases.md` adjustments only specify what content to write in the
otherwise-required sections.
