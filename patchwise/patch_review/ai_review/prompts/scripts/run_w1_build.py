...
# W=1 build evidence must be real compiler/check output, not a Kconfig
# syncconfig transcript.  run_w1_build.py seeds a fresh defconfig and then runs
# olddefconfig before each refreshed build; after that, seeing syncconfig prompt
# markers in the captured build output means the verification artifact is not a
# clean non-interactive build log and must be regenerated.
...
def _run_make(project: Path, args: list[str]) -> subprocess.CompletedProcess[str]:
    ...
    # The kernel build re-runs `syncconfig` whenever a Kconfig source is newer
    # than include/config/auto.conf, and that syncconfig is deliberately
    # INTERACTIVE (see the kernel top Makefile: "Do not use $(call cmd,...)
    # here. That would suppress prompts from syncconfig").  On a freshly synced
    # linux-next tree it can prompt for dozens of new symbols.  With
    # stdin=DEVNULL each prompt reads EOF -> "Error in reading or end of file",
    # the auto.conf step degrades, and has_interactive_kconfig() then rejects
    # the whole build.  Feed an endless stream of blank lines so every prompt
    # takes its default (identical to what olddefconfig would pick) and the
    # build proceeds non-interactively.  `yes ''` closes its own pipe when make
    # exits; we do not wait on it.
