...
"""
    ...
    def __init__(self, patch_review, review_result: str):
        ...
        # Add sparse verification tool to the agent
        self._add_sparse_tool()

    def _add_sparse_tool(self):
        """Add run_sparse tool to the agent's available tools."""
        sparse_tool = {
            "type": "function",
            "function": {
                "name": "run_sparse",
                "description": "Run sparse on the modified files to verify fixes. Returns sparse output showing remaining issues or success.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "file_path": {
        ...
        if hasattr(self.agent, 'tools'):
            self.agent.tools.append(sparse_tool)
        
        # Register the tool handler
        if hasattr(self.agent, 'tool_handlers'):
            self.agent.tool_handlers['run_sparse'] = self._handle_run_sparse

    def _handle_run_sparse(self, file_path: str = None) -> str:
        """Handle run_sparse tool call from the AI.
        
        Args:
            file_path: Optional specific file to check
            
        Returns:
            Sparse output or success message
        """
        ...
        try:
            ...
            if proc.returncode != 0:
                return "Failed to get modified files"
            
            modified_files = [f.strip() for f in stdout.decode().splitlines() if f.strip()]
            
            if file_path:
                # Check specific file
                files_to_check = [file_path] if file_path in modified_files else []
            else:
                # Check all modified C files
                files_to_check = [f for f in modified_files if f.endswith(('.c', '.h'))]
            
            if not files_to_check:
                return "No C files to check"
            
            # Run sparse on each file
            all_output = []
            has_warnings = False
            
            for file in files_to_check:
                # Run sparse (this is a simplified version - actual sparse command may vary)
                proc = self.patch_review.docker_manager.run_command(
                    ["make", f"C=2", file.replace('.c', '.o')],
                    cwd=kernel_dir
                )
            ...
            if not has_warnings:
                return "✓ SUCCESS: All sparse issues fixed! No warnings or errors remain."
            else:
                combined_output = "\n".join(all_output)
                return f"Sparse output:\n{combined_output}\n\nIssues remain. Please fix them and run sparse again."
                
        except Exception as e:
            self.logger.error(f"Error running sparse: {e}")
            return f"Error running sparse: {str(e)}"
