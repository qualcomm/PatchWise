...
def get_selected_reviews_from_args(args: argparse.Namespace) -> set[str]:
    ...
    deep_review_names = {cls.__name__ for cls in DEEP_REVIEWS}
    if getattr(args, "all_reviews", False):
        # Deep review is opt-in only (it needs a full kernel tree and runs the
        # heavy prompt-backed flow); never fold it into the broad --all-reviews
        # group.  Request it explicitly via --deep-review or --reviews.
        group_sets.append(
            {
                cls.__name__
                for cls in AVAILABLE_PATCH_REVIEWS
    ...
    if getattr(args, "deep_review", False):
        group_sets.append(set(deep_review_names))
