...
This project enforces code quality through Ruff with the following rules:
...
- **Line length**: Maximum of 88 characters (Ruff's default, following Black's convention)

#### Import Rules:
- **Use absolute imports**: Enforced by rule TID252. Import from the full module path (e.g., `from patchwise.module import function`)
- **Relative imports from siblings are acceptable**: You can use relative imports within the same package level
...
- Exceptions: `__init__` functions don't require return type annotations, dummy parameters may omit type hints
